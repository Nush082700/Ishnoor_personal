$(document).ready(function () {
  ...
  document.getElementById("giftLink").addEventListener("click", function () {
    swal("Successful !", "Check your email for the surprise!", "success");
  });
});
...
function sort(n) {
  ...
  for (i = 0; i < _photo.length; i++) {
    _photo[i].className = _photo[i].className.replace(
      /\s*photo_center\s*/,
      " "
    );
  ...
  for (var i = 0; i < photos_left.length; i++) {
    ...
    photos_left[i].style["transform"] =
      "rotate(" + random([-150, 150]) + "deg) scale(1)";
    photos_left[i].style["-webkit-transform"] =
      "rotate(" + random([-150, 150]) + "deg) scale(1)";
  }
  for (var i = 0; i < photos_right.length; i++) {
    ...
    photos_right[i].style["transform"] =
      "rotate(" + random([-150, 150]) + "deg) scale(1)";
    photos_right[i].style["-webkit-transform"] =
      "rotate(" + random([-150, 150]) + "deg) scale(1)";
  }
  var navs = select(".i");
...
function addPhotos() {
  ...
  for (i = 0; i < data.length; i++) {
    ...
    nav.push(
      '<span id="nav_' +
        i +
        '" class="i" onclick ="turn(select(\'#photo_' +
        i +
        "'))\">&nbsp;</span>"
    );
  }
...
function turn(elem) {
  ...
  if (/photo_front/.test(cls)) {
    ...
    select("#nav_" + n).className = select("#nav_" + n).className.replace(
      /\s*i_back\s*/,
      " "
    );
  }
